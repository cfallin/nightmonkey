...
the crash-on-failure of the in-process lane.

Plain `cargo build` works for the Rust crates with `SPIDERMONKEY_DIST` set in
the environment: the compiler's build script reads the engine's opcode table
from `$SPIDERMONKEY_DIST/include-private/vm/Opcodes.h`.
...
Compile a program, in one command:
...
- Layout facts the compiler bakes into generated code are pinned by
  ...
  miscompiling. The opcode table is regenerated from `Opcodes.h` on every
  build.
