...
3. Diff `vm/Opcodes.h` against the previous engine and review
   `compiler/src/opsem.rs` and the lowerings against the change. The
   compiler's opcode enum, lengths and stack effects are generated from the
   header and checked in per engine version (`compiler/src/opcodes/`, see the
   README's "Engine versions"); every build checks the selected table against
   the engine's header, digest included, so any change to it, structural or
   only in meaning, is refused until the table is regenerated (or a new
   version added, with the lowering differences gated on its feature).
